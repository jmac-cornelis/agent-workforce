...
class MonitorConfig:
    ...
    def from_yaml(cls, yaml_input: str | Mapping[str, Any]) -> 'MonitorConfig':
        ...
        valid_affects_versions = (
            [str(v) for v in raw_versions] if isinstance(raw_versions, list) else []
        )
...
def determine_actions(
    ...
    valid_av = (
        set(config.valid_affects_versions) if config.valid_affects_versions else set()
    )
    ...
    def decide_action(field_name: str, severity: str) -> None:
        ...
        if config.learning_enabled:
            predicted_value, confidence = _predict_for_field(
                learning_store,
                field_name,
                validation,
            )
