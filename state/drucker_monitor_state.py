...
class DruckerMonitorState:
    def __init__(self, db_path: str = 'data/drucker_monitor_state.db'):
    ...
    def _require_conn(self) -> sqlite3.Connection:
        if self.conn is None:
            raise RuntimeError('DruckerMonitorState connection is closed')
        return self.conn
    ...
    def _init_db(self) -> None:
        ...
        with self._lock:
            ...
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_drucker_processed_project '
                'ON processed_tickets(project)'
            )
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_drucker_history_ticket '
                'ON validation_history(ticket_key, timestamp DESC)'
            )
            cursor.execute(
                'CREATE INDEX IF NOT EXISTS idx_drucker_history_project '
