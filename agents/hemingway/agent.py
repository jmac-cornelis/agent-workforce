##########################################################################################
#
# Module: agents/hemingway_agent.py
#
...
class HemingwayDocumentationAgent(BaseAgent):
    '''
    Deterministic-first documentation coordinator.

    Hemingway analyzes documentation impact, synthesizes source-backed internal
    ...
    def __init__(self, project_key: Optional[str] = None, **kwargs):
        ...
        if not instruction:
            raise FileNotFoundError(
                'agents/hemingway/prompts/system.md is required but not found. '
                'The Hemingway Documentation Agent has no hardcoded fallback prompt.'
            )

        config = AgentConfig(
            name='hemingway_documentation',
    ...
    def _load_prompt_file() -> Optional[str]:
        ...
        if os.path.exists(prompt_path):
            try:
                ...
                log.warning(f'Failed to load Hemingway agent prompt: {e}')
        return None
    ...
    def run(self, input_data: Any) -> AgentResponse:
        ...
        '''
        log.debug(f'HemingwayDocumentationAgent.run(input_data={input_data})')
    ...
    def create_review_session(self, record: DocumentationRecord) -> ReviewSession:
        '''
        Convert Hemingway patches into a review-gated publication session.
        '''
    ...
    def publish_approved(
        ...
        '''
        Publish already-approved Hemingway actions through the shared ReviewAgent.
        '''
    ...
    def _derive_title(self, request: DocumentationRequest) -> str:
        ...
        return 'Hemingway Documentation Draft'

    def _default_target_file(self, title: str) -> str:
        slug = re.sub(r'[^a-z0-9]+', '-', title.casefold()).strip('-') or 'hemingway-doc'
    ...
    def _read_source_file(
        ...
        if result.is_error:
            log.warning(f'Failed to read Hemingway source file {source_path}: {result.error}')
            return None
    ...
    def _format_record_summary(
        ...
        '''
        Format the high-level Hemingway summary used in records and workflow output.
        '''
        lines = [
            f'# Hemingway Documentation Plan: {request.title or self._derive_title(request)}',
            '',
            '## Summary',
            f'- Documentation class: `{request.doc_type}`',
    ...
    def _maybe_run_sphinx_validation(
        ...
        with tempfile.TemporaryDirectory(prefix='hemingway-sphinx-') as temp_dir:
            try:
