...
from agents.hemingway.models import DocumentationRecord, PublicationRecord

# Logging config - follows jira_utils.py pattern
log = logging.getLogger(os.path.basename(sys.argv[0]))


class HemingwayRecordStore:
    '''
    JSON + Markdown persistence for Hemingway documentation records.

    Records are stored at:
        data/hemingway_docs/<DOC_ID>/record.json
        data/hemingway_docs/<DOC_ID>/summary.md
    '''
    ...
    def save_record(
        ...
        log.info(f'Saved Hemingway documentation record {doc_id} to {record_dir}')
        return summary
    ...
    def get_record(self, doc_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            log.error(f'Failed to load Hemingway record {doc_id}: {e}')
            return None
        ...
        if markdown_path.exists():
            try:
                ...
                log.warning(f'Failed to read Hemingway summary markdown {markdown_path}: {e}')
