...
- Searches across Hemingway records, Confluence pages, and GitHub repositories for existing documentation
...
Hemingway has 7 Shannon commands registered:
...
| `/search-docs` | Search Hemingway documentation records | No | `@Shannon /search-docs query "build guide" doc_type engineering_reference` |
...
Use `/help` in the Hemingway Teams channel to see all parameters for each command.
...
Hemingway exposes a REST API on port 8203.
...
Hemingway supports 3 documentation types with corresponding prompt files:
...
| `confluence-publish` | Publish markdown to Confluence with diagram rendering | `hemingway-agent confluence-publish --input-file docs/PLAN.md --title "My Doc" --space 238190621 --parent-id 656572464` |
...
hemingway-agent confluence-publish \
...
- **Container run command**:
  ...
  ```bash
  podman run -d --name hemingway -p 8203:8203 \
    --env-file deploy/env/shared.env \
    --env-file deploy/env/llm.env \
    -v $(pwd)/config:/app/config:ro,Z \
    -v $(pwd)/data/hemingway:/data/hemingway:Z \
    localhost/cornelis/agent-workforce:latest \
    uvicorn agents.hemingway.api:app --host 0.0.0.0 --port 8203
  ```

- **Verify**: `curl http://localhost:8203/v1/health`
- **Systemd**: `deploy/systemd/hemingway.service`
- **Cross-container**: Shannon reaches Hemingway at `http://host.containers.internal:8203`
