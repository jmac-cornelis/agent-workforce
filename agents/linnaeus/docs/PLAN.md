...
- replacing Babbage version mapping logic
...
- Hedy establishes release facts
- Babbage establishes version mapping facts
...
## Architecture
### Core design
Linnaeus should be split into these concerns:
...
### Identity model
Linnaeus should anchor on stable identifiers, not fuzzy summaries.

Primary identity sources:
- FuzeID / internal build ID from Josephine
- Jira issue key
- test run or test cycle ID from Faraday / Fuze Test
- release ID from Hedy
- external version identifier from Babbage
...
- release context should attach to the exact build or build set promoted by Hedy
...
Josephine and Faraday should expose machine-readable records with exact IDs, artifact references, and status summaries so Linnaeus does not need to scrape logs.

### 3. Structured release and version outputs
Hedy and Babbage should expose release and version mapping records directly so Linnaeus can link them deterministically.
...
- Faraday and Hedy expose stable test and release records
