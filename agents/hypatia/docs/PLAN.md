# Hypatia Documentation Agent Plan

## Summary
Hypatia should be the documentation agent for the platform. Its v1 job is to turn source changes, build and test facts, traceability context, release context, and meeting-derived clarifications into durable engineering and user-facing documentation updates.

Hypatia should not be a prose generator detached from implementation truth. It should produce documentation from authoritative system records.
...
- replacing Hypatia with generic LLM-generated markdown dumps
...
- Hypatia turns those sources into durable documentation artifacts

## Triggering model
- Hypatia should run as an always-on documentation service.
...
## Architecture
### Core design
Hypatia should be split into these concerns:
...
Hypatia should be grounded in the existing documentation substrate already present in `fuze`:
...
- Hypatia should update existing doc structures when possible instead of creating parallel documentation silos
...
Hypatia will be stronger if the platform exposes documentation-friendly inputs.
...
Exit criteria:
- Hypatia can identify affected docs for a scoped change
