...
- replacing Mercator version mapping
- replacing Berners-Lee traceability ownership
...
- Galileo, Curie, Faraday, and Tesla provide validation and environment evidence
- Mercator provides internal-to-external version mapping
- Berners-Lee provides issue, requirement, build, and test traceability
- Humphrey decides whether a build should become a release candidate, advance stage, pause, or be blocked


## Triggering model
- Humphrey should run as an always-on release control-plane service.
...
## Architecture
### Core design
Humphrey should be split into these concerns:
...
### Release-state model
Humphrey should treat the existing Fuze release model as canonical for v1.
...
V1 rule:
- Humphrey may use the richer internal state model operationally, but must clearly distinguish:
  - public release stages
  - internal workflow states
  - terminal blocked or rejected outcomes

### Release candidate model
Humphrey should create a `ReleaseCandidate` that contains:
...
sequenceDiagram
    ...
    participant H as Humphrey API
    participant J as Josephine
    participant F as Faraday
    participant B as Mercator
...
## Execution topology
### Where Humphrey runs
- run Humphrey on standard internal service infrastructure
- Humphrey is a control-plane service, not a build or test worker
- Humphrey may trigger downstream work, but should not execute builds or tests directly

### How release flow happens
1. Josephine emits build completion
2. Mercator maps version context
3. test agents attach validation evidence
4. Berners-Lee attaches traceability and defect context
5. Humphrey evaluates release readiness
6. Humphrey creates a release candidate or blocks it
7. when approved, Humphrey promotes the release through Fuze-compatible release operations
8. Humphrey emits release records and downstream notifications
...
Humphrey can reuse `fuze` release capabilities, but the following changes would make it a stronger release substrate.
...
Separate release-state persistence from optional automations such as Jira and test triggering so Humphrey can call them intentionally and observe their results independently.
...
Exit criteria:
- Humphrey can evaluate a build and produce a release decision
...
Exit criteria:
- Humphrey can distinguish release readiness by target scope
...
- Mercator owns external version mapping, not Humphrey
- Berners-Lee owns traceability records, not Humphrey
