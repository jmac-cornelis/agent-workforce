...
- Humphrey owns release decisions and promotion workflows
- Mercator owns version mapping and version lineage
- Berners-Lee consumes version mappings as part of the traceability graph


## Triggering model
- Mercator should run as an always-on version-mapping service.
...
## Architecture
### Identity model
Mercator should explicitly distinguish:
...
- consumes Mercator mappings when creating release candidates and promotions
...
Mercator can work alongside `fuze` as-is, but the following improvements would make mapping cleaner.
...
Improve structured representation of replaced or superseded releases so Mercator does not need to infer lineage from loosely structured release history fields.
...
- tighten integration with Humphrey release candidate workflows
- improve query interfaces for downstream traceability and documentation

Exit criteria:
- Humphrey can consume version mappings without ambiguity
- Berners-Lee and Hemingway can read mappings as stable system records
...
- Humphrey owns release decisions, not Mercator
