...
class FeaturePlanBuilderAgent(BaseAgent):
    ...
    @staticmethod
    def _load_plan_building_instructions() -> str:
        '''Load the plan building instructions prompt from config/prompts/.'''
        prompt_path = os.path.join(
            'config', 'prompts', 'plan_building_instructions.md'
        )
        if os.path.exists(prompt_path):
            try:
                with open(prompt_path, 'r', encoding='utf-8') as f:
                ...
                log.warning(f'Failed to load plan building instructions: {e}')
        raise FileNotFoundError(
            'config/prompts/plan_building_instructions.md is required but not found.'
        )
    ...
    def _build_plan_prompt(
        ...
        # Load the plan building instructions from external prompt file
        # and substitute the {project_key} placeholder.
        instructions_template = self._load_plan_building_instructions()
        lines.append(instructions_template.replace('{project_key}', project_key))
