...
class FeaturePlanningOrchestrator(BaseAgent):
    ...
    def run(self, input_data: Any) -> AgentResponse:
        ...
        self._requested_by = input_data.get('requested_by', '')
        self._approved_by = input_data.get('approved_by', '')
        self._correlation_id = input_data.get('correlation_id', '')
    ...
    def _resolve_initiative(self, project_key: str, feature_name: str,
        ...
        try:
            ...
            initiative_context = self._resolve_jira_execution_context('bulk_create')
            ...
            result = create_ticket(
                    ...
                    actor_mode=initiative_context.get('actor_mode'),
                    requested_by=initiative_context.get('requested_by'),
                    approved_by=initiative_context.get('approved_by'),
                    policy_rule=initiative_context.get('policy_rule'),
                    correlation_id=initiative_context.get('correlation_id'),
                )
    ...
    def _resolve_jira_execution_context(
        self,
        action_class: str,
        *,
        risk: str = 'low',
        ...
        '''
        Resolve Jira actor context for direct plan-execution writes.
        '''
        plan = self.state.jira_plan or {}
        project_key = str(plan.get('project_key') or self.state.project_key or '').strip()
        feature_name = str(plan.get('feature_name') or self.state.feature_request or 'plan').strip()
        default_correlation = f'feature-plan:{project_key}:{feature_name}'
        default_correlation = re.sub(r'[^a-zA-Z0-9:_-]+', '-', default_correlation).strip('-')
        ...
        resolution = resolve_jira_actor(
            ...
            requested_by=getattr(self, '_requested_by', '') or None,
            approved_by=getattr(self, '_approved_by', '') or None,
            correlation_id=getattr(self, '_correlation_id', '') or default_correlation,
        )
    ...
    def _phase_execution(self) -> AgentResponse:
        ...
        errors: List[str] = []
        ticket_context = self._resolve_jira_execution_context('ticket_tree_create')
        link_context = self._resolve_jira_execution_context('link_update')
        ...
        for epic_idx, epic_data in enumerate(plan.get('epics', [])):
            ...
            try:
                ...
                epic_result = create_ticket(
                    ...
                    product_family=product_family,
                    actor_mode=ticket_context.get('actor_mode'),
                    requested_by=ticket_context.get('requested_by'),
                    approved_by=ticket_context.get('approved_by'),
                    policy_rule=ticket_context.get('policy_rule'),
                    correlation_id=ticket_context.get('correlation_id'),
                )
                ...
                for i in range(len(epic_story_keys) - 1):
                    ...
                    try:
                        link_result = link_tickets(
                            ...
                            to_key=to_key,
                            link_type='Relates',
                            actor_mode=link_context.get('actor_mode'),
                            requested_by=link_context.get('requested_by'),
                            approved_by=link_context.get('approved_by'),
                            policy_rule=link_context.get('policy_rule'),
                            correlation_id=link_context.get('correlation_id'),
                        )
