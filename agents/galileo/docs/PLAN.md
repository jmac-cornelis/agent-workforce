...
### V1 shape
- `galileo-api`: accepts planning requests and exposes plan records
- `galileo-worker`: resolves planning inputs into normalized `TestPlan` outputs
...
## Triggering model
- Galileo should run as an always-on planning service.
...
## Architecture
### Core design
Galileo should be split into three internal concerns:
...
### Trigger model
Galileo should support these v1 trigger classes:
...
Galileo should integrate with Fuze Test using the existing selection and execution model described in:
...
- Galileo produces planning intent, not direct repo mutations
- Curie should be able to derive explicit suite inputs from Galileo plans
...
## Planning topology
### Where Galileo runs
- run `galileo-api` on normal internal service infrastructure
- planning does not require lab-resident worker hosts
- Galileo should run close to the event and metadata systems it consumes

### How planning happens
- Galileo receives a build event or direct planning request
- Galileo resolves trigger class, scope, and coverage intent
- Galileo selects a `TestPlan`
- Galileo emits a durable planning record for Curie, Faraday, Tesla, and downstream traceability consumers
...
- when environment constraints prevent a requested HIL path, Galileo should either:
  - degrade to an approved fallback plan, or
  - fail planning with an explicit reason


## Queueing and planning lifecycle
- put an explicit queue between Galileo API and planning workers when planning load warrants it
...
Galileo should use Fuze Test as it exists, but the following changes would make it a much better planning substrate.
...
- add build-ID linkage from Josephine to Galileo
...
Exit criteria:
- Curie and Faraday can consume Galileo plans without ambiguity
- Galileo can preview and explain a plan before execution
...
## Assumptions
- Fuze Test in [atf](/Users/johnmacdonald/code/cornelis/atf) is the execution substrate downstream of Galileo planning
- Josephine provides the build ID, artifact context, and build-completion events Galileo consumes
- Curie materializes executable test inputs from Galileo plans
