...
def main():
    ...
    # Highlight AI co-authors
    ai_authors = ['developer-opus', 'developer-codex', 'developer-gpt', 'Claude:claude-4.5-opus', 'claude-sonnet-4-20250514', 'CornelisAI']
    for ai in ai_authors:
        # Avoid double replacement if already run
        content = re.sub(fr'\b({re.escape(ai)})\b', r'🤖 **\1**', content)
        # Fix potential double replacement
        content = content.replace('🤖 **🤖 **', '🤖 **').replace('** **', '**')

    # Build the bug table (static data from instructions, but author for 63325 fixed to Bob Cernohous)
